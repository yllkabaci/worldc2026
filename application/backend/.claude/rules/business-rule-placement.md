...
| Scoring arithmetic, multipliers, void cases | **Domain** scoring service / `ScoringRuleSet` (see `scoring-engine.md`) |
...
- **Validator (allowed):** required fields, types, value ranges (goals `0-20` per BR-010, yellow cards `0-20`, red cards `0-10`, subs `0-5`), string formats, enum membership.
- **Domain (required):** "prediction allowed only before the deadline" (BR-004/BR-007), "match must exist and be in a predictable state", "one active prediction per match" (BR-005), "points never negative" (BR-002), "minute-of-first-goal void on 0-0" (BR-026), stage multipliers, rule versioning.
...
- A rule that should vary (point values, multipliers, prediction window) is **configuration data on a versioned aggregate**, not a hardcoded literal. Past matches are scored by the rule version active at settlement (business doc §8.2).
