...
4. **Configurable, not hardcoded**: point values/multipliers/windows are config on the versioned rule set, not literals.

### Scoring correctness (verify against the spec)
5. **Base tiers**: exact (default 3), correct winner (1), correct draw (1), miss (0) - configurable values respected.
6. **Bonuses**: each enabled bonus added correctly; respect each bonus's range and enabled-flag.
7. **Multiplier**: applied to the **match total**, not to individual bonuses (BR-001).
8. **Void cases**: minute-of-first-goal void on 0-0 (BR-026); cancelled match -> all predictions void, no points/penalty (BR-008); penalty bonus excludes shootouts (BR-027).
9. **Determinism & purity**: scoring is a pure function (no I/O, clock, randomness); same inputs -> same output.
